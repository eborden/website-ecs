...
export function init(ECS, colliders) {
  const left = makeBox(ECS, {x: -99, y: 0, width: 100, height: screen.height})
  ECS.addEntity(left)
  colliders.push(left)

  const right = makeBox(ECS, {x: camera.maxX - 1, y: 0, width: 100, height: screen.height})
  ECS.addEntity(right)
  colliders.push(right)

  const floor = makeBox(ECS, {x: 0, y: screen.height - 1, width: camera.maxX, height: 100})
  ECS.addEntity(floor)
  colliders.push(floor)

  const ceiling = makeBox(ECS, {x: 0, y: -99, width: camera.maxX, height: 100})
