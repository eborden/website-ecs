...
export function init(ECS, colliders) {
  const left = makeBox(ECS, {x: -50, y: 0, width: 50, height: screen.height})
  ECS.addEntity(left)
  colliders.push(left)

  const right = makeBox(ECS, {x: camera.maxX, y: 0, width: 50, height: screen.height})
  ECS.addEntity(right)
  ...
  const ceiling = makeBox(ECS, {x: 0, y: -50, width: camera.maxX, height: 50})
